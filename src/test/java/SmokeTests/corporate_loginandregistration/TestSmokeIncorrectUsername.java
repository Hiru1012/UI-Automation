...
public class TestSmokeIncorrectUsername extends DriverFactory {

    public static String correctPassword = propertyFileReader.getProperty("config", "Password");
    ...
    @Test(description = "Checking login with random incorrect email", priority = 4)
    @Description("Testing login with a random incorrect email")
    @Feature("Login")
    public void checkLoginWithRandomIncorrectEmail() throws InterruptedException {
        ...
        PageFactory.loginPage.login(randomIncorrectEmail, correctPassword);
